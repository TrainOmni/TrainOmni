...
| Offline dense-logit KD | current-tree automated; historical pre-fix real VLM; revalidation pending | custom-required Objective ABI; PyTorch FP32 CE/KL | schema-v3 cache/alignment/numeric/gradient tests are current; historical full-vocab CUDA run predates the corrected schema |
...
| Hash-pinned safetensors sidecar cache | current-tree automated | schema-v3 index/shard digests plus full expanded input IDs, full attention mask, absolute target positions/IDs, producer and branch bindings; schema-v2 and corruption fail before model forward |
