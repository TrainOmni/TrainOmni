"""Canonical image/video samples to Qwen-vision and MiniCPM tensors."""
...
class ModelIO:
    ...
    def _resolve_image(self, raw: Any):
        ...
        if not path.is_absolute():
            if self.task_root is None:
                raise SpecError("relative visual path requires a task root")
            path = self.task_root / path
        path = path.resolve()
        if not path.is_file():
            raise SpecError(f"visual frame does not exist: {path}")
        with Image.open(path) as image:
            return image.convert("RGB").copy()

    def _visuals(self, sample) -> list[Any]:
        ...
        for block in blocks:
            if block.kind == "image":
                visuals.append(self._resolve_image(block.value))
            elif block.kind == "video":
                if not isinstance(block.value, (list, tuple)) or not block.value:
                    raise SpecError(
                        "predecoded video must be a non-empty ordered frame-path list"
                    )
                visuals.extend(self._resolve_image(frame) for frame in block.value)
        if not visuals:
            raise SpecError("real VLM sample requires image or video frames")
        return visuals

    def _vision_inputs(self, sample) -> dict[str, torch.Tensor]:
        visuals = self._visuals(sample)
        values = self.processor(images=visuals, return_tensors="pt")
        return {
            "pixel_values": values["pixel_values"],
            "image_grid_thw": values["image_grid_thw"],
            "image_counts": torch.tensor([len(visuals)], dtype=torch.long),
        }
    ...
    def _caption_values(self, sample) -> tuple[str, str]:
        ...
        return str(sample.metadata.get("prompt", "Describe the visual input.")), answers[0]
...
def descriptor() -> ModuleDescriptor:
    return ModuleDescriptor(
        module_id=ModuleId.parse("model_io:validation/qwen35_vision_minicpm5_ext@2"),
        config_type=Config,
        factory=_factory,
